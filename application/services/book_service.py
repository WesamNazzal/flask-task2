...
class BookService(BaseService[Table]):
    ...
    def borrow_book(self, book_id: int, member_id: int) -> Tuple[Dict[str, Any], int]:
        if not (book := self.repo.get_book_by_id(book_id)):
        ...
        if not (updated_book := self.repo.update_book(book_id, update_data)):
    ...
    def update_book(self, book_id: int, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
        ...
        return self.repo.update(book_id, data), 200
    ...
    def get_borrowed_book_with_member(self, book_id: int) -> Tuple[Dict[str, Any], int]:
